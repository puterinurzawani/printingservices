...
class ImagePrinting extends StatefulWidget {
  final String userId;
  ImagePrinting({required this.userId});

  @override
  _ImagePrintingState createState() => _ImagePrintingState();
}

class _ImagePrintingState extends State<ImagePrinting> {
  int activeStep = 0;
  var sizeValue;
  ...
  callApiPayment() {
    ...
    var jsons = {
      ...
      "fileBase64Image": imageWithBase64,
    ...
    HttpAuth.postApi(jsons: jsons, url: 'display_size.php').then((value) {
      if (value.statusCode == 200) {
        ...
        sizeDisplay = sizedisplay.sizeDisplay
            .where((element) => element.typeId.toString() == "3")
            .toList();
        isLoading = false;
        setState(() {});
      }
  ...
  void _openFileUpload(isFromPayment) async {
    if (showImageFilePayment.length < 1) {
      try {
        FilePickerResult? result = Platform.isAndroid
            ? await FilePicker.platform.pickFiles(
                type: isFromPayment ? FileType.any : FileType.media,
                allowMultiple: isFromPayment ? false : true,
  ...
  void insertFileInArray(
    ...
    if (isFromPayment == false) {
      showImageFile.add(File(additionalPath ?? file.path!));
      imageWithBase64.add(base64ImageUploadSizePhoto);
      typeOfExtension.add(file.extension!);
    } else {
      showImageFilePayment.add(File(additionalPath ?? file.path!));
                  ...
                  SizedBox(
                    ...
                    child: Mainbutton(
                      onTap: () {
                        ...
                        if (fileName.isEmpty) {
                          toastToUser(msg: "Please upload the image");
                          return;
                        }
                        activeStep = 1;
